function dydt = odefcnDeffuantMu05_asymptotics(t,P,L,coeffs)

% opinion space trucated at L (or sort of L-1) instead of 1.

n_classes = length(coeffs(1,:));
n = length(P)/n_classes;
P_temp = reshape(P,n,n_classes)';
B_temp = zeros(n_classes,n,n);
eps_state = (n/L); %# states between 0 and 1
...
v = [zeros(n_classes,n),P_temp,ones(n_classes,n)]; %clever
vu = v(:,3:(length(v)))/2; 
v0 = v(:,2:(length(v)-1));
vl = v(:,1:(length(v)-2))/2;
for j = 1:(n)
    ...
    B_temp(:,:,j) = (L/n).*(vl(:,(n+1-j):2:(3*n-1-j)).*Nl(:,(n-j+1):(2*n-j))...
