...
% Pineda rule
    epsilons = 1/2./deltas;
    n_clusters_Pineda = round(0.44 ./ epsilons);
    % Biffurcation poitns
    k = 1;
    for i=2:9
        clusters_indecies_Pineda(k) = find(n_clusters_Pineda == i,1);
        k = k+1;
    end
    bif_points_Pineda = deltas(clusters_indecies_Pineda);
...
p_Pineda = scatter(bif_points_Pineda, [2:1:9], 60,'g','o','filled', 'LineWidth',1.5);
hold on

for i = [1:1:8]
    plot([bif_points_Pineda(i) bif_points_Pineda(i)], [deltas_discrete(i)-0.5 deltas_discrete(i)+0.5],'g')
    hold on
end
...
xlim([1,10])
ylim([0,9.5])
legend([p_blueline(1) p_LS(1) p_Pineda(1)],'Numerical soln.','Lin. stability approx. ','Pineda approx',...
        'Location', 'southeast',...
